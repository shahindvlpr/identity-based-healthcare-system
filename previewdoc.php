...
<?php
    ...
    if (isset($_POST['submit']) && isset($_FILES['image'])){
        ...
        if($password == $conformpassword && strlen($password)>=8){
            $query1 = mysqli_query($conn,"SELECT * FROM patient where p_nid = $nid");
            $per = mysqli_fetch_array($query1);
            ...
            if ($error === 0) {
                ...
                if (in_array($img_ex_lc, $allowed_exs)) {
                    ...
                    mysqli_query($conn,"update person set occupation = 'Doctor', image = '$new_img_name' where nid = '$nid'");
                    mysqli_query($conn,"insert into doctor (d_nid,dmdc_id,PASSWORD) values ('$nid','$dmdc_id','$password')");
                        if($per == NULL){
...
<div class="container1">
    ...
    <div class="row" style="left: 0px;right: 0px; margin:auto; position:relative; width: 100%;">
        ...
        <div class="input-group mb-3">
             <div class="input-group-prepend">
                 <span class="input-group-text" id="basic-addon3">DMDC ID</span>
             </div>
             <input type="text" class="form-control" value="<?php echo $dmdc_id ?>" disabled>
         </div> 
    ...
    <form action="" method="POST" enctype="multipart/form-data">
          <input type="hidden" name="nid" class="form-control" value="<?php echo $nid; ?>">
            <!-- <div class="input-group mb-3">
                <div class="input-group-prepend">
                    <span class="input-group-text" id="basic-addon3">Visiting Fee</span>
                </div>
            ...
            <div class="input-group mb-3">
                ...
                <input type="password" class="form-control" name="conformpassword" required>
            </div>
